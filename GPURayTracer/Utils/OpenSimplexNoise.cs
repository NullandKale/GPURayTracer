...
namespace Simplex
{
    ...
    public class Noise
    {
        ...
        static Noise()
        {
            rand = new byte[permOriginal.Length];
            Simplex.Noise.permOriginal.CopyTo(rand, 0);
        }
        ...
        internal static float Generate(float x)
        {
            ...
            float t0 = 1.0f - x0 * x0;
            t0 *= t0;
            n0 = t0 * t0 * grad(rand[i0 & 0xff], x0);

            float t1 = 1.0f - x1 * x1;
            t1 *= t1;
            n1 = t1 * t1 * grad(rand[i1 & 0xff], x1);
        ...
        public static void GenerateKernel(Index1 index, int width, int height, float scaleXY, ArrayView<float> dataIn, ArrayView<float> dataOut, ArrayView<byte> rand, int method, float arg0, float arg1, float arg2, float arg3)
        {
            if (method != map)
            {
                ...
                if (method == add)
                {
                    dataOut[index] = dataIn[index] + (val * arg0);
                }
            ...
            {
                dataOut[index] = Map(dataIn[index], arg0, arg1, arg2, arg3);
            }
        }
        ...
        internal static float Generate(float x, float y, float z)
        {
            ...
            {
                t0 *= t0;
                n0 = t0 * t0 * grad(rand[ii + rand[jj + rand[kk]]], x0, y0, z0);
            }
            ...
            {
                t1 *= t1;
                n1 = t1 * t1 * grad(rand[ii + i1 + rand[jj + j1 + rand[kk + k1]]], x1, y1, z1);
            }
            ...
            {
                t2 *= t2;
                n2 = t2 * t2 * grad(rand[ii + i2 + rand[jj + j2 + rand[kk + k2]]], x2, y2, z2);
            }
            ...
            {
                t3 *= t3;
                n3 = t3 * t3 * grad(rand[ii + 1 + rand[jj + 1 + rand[kk + 1]]], x3, y3, z3);
            }
